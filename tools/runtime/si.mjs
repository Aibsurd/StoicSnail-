...
function getInsights(limit = 20) {
  return db
    .prepare(`SELECT * FROM insights ORDER BY importance DESC, created_at DESC LIMIT ?`)
    .all(limit);
}
...
function updateContext(project, context) {
  ...
  if (existing) {
    db.prepare(
      `UPDATE working_context SET context = ?, updated_at = CURRENT_TIMESTAMP WHERE project = ?`,
    ).run(context, project);
  } else {
    db.prepare(`INSERT INTO working_context (project, context) VALUES (?, ?)`).run(
      project,
      context,
    );
  }
...
function recordPattern(pattern) {
  ...
  if (existing) {
    db.prepare(
      `UPDATE patterns SET frequency = frequency + 1, last_seen = CURRENT_TIMESTAMP WHERE pattern = ?`,
    ).run(pattern);
  } else {
...
try {
  switch (cmd) {
    ...
    case "start": {
      ...
      db.prepare(
        `UPDATE sessions SET status = 'ended', ended_at = CURRENT_TIMESTAMP WHERE status = 'active'`,
      ).run();
