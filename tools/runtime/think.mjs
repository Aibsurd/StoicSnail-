...
try {
  if (THOUGHT_KINDS.includes(command)) {
    ...
    console.log(
      `\x1b[32m✓\x1b[0m ${icons[command]} [#${result.lastInsertRowid}] ${command}: ${text}`,
    );
  } else if (command === "log") {
    ...
    const rows = db
      .prepare(`SELECT * FROM thoughts WHERE kind = 'blocker' AND resolved = 0 ORDER BY id`)
      .all();
    if (rows.length === 0) {
      console.log("\x1b[32m✓ No active blockers\x1b[0m");
      process.exit(0);
    }
    ...
    if (rows.length === 0) {
      console.log("(empty reasoning chain)");
      process.exit(0);
    }
    ...
    if (!sessionTag) {
      ...
      const tags = db
        .prepare(
          `SELECT DISTINCT session_tag, COUNT(*) as cnt FROM thoughts WHERE session_tag IS NOT NULL GROUP BY session_tag`,
        )
        .all();
      if (tags.length === 0) {
        console.log("(no tagged sessions)");
        process.exit(0);
      }
      ...
      const rows = db
        .prepare(`SELECT * FROM thoughts WHERE session_tag = ? ORDER BY id`)
        .all(sessionTag);
    ...
    if (!id) {
      console.error("Usage: think resolve <id>");
      process.exit(1);
    }
    ...
    const byKind = db
      .prepare(
        `SELECT kind, COUNT(*) as c, AVG(confidence) as avg_conf FROM thoughts GROUP BY kind`,
      )
      .all();
    const problems = db
      .prepare(`SELECT content FROM thoughts WHERE kind = 'problem' ORDER BY id`)
      .all();
    const conclusions = db
      .prepare(`SELECT content FROM thoughts WHERE kind = 'conclusion' ORDER BY id`)
      .all();
    const openBlockers = db
      .prepare(`SELECT content FROM thoughts WHERE kind = 'blocker' AND resolved = 0`)
      .all();
