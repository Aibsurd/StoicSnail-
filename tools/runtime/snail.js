...
const TOOLS = {
  fetch: {
    ...
    run: () =>
      spawn("node", [join(__dirname, "fetch.mjs"), ...process.argv.slice(3)], { stdio: "inherit" }),
  },
  scrape: {
    desc: "Web scraper using CSS selectors",
    usage: "snail scrape <url|file> <selector> [attr]",
    run: () =>
      spawn("node", [join(__dirname, "scrape.mjs"), ...process.argv.slice(3)], {
        stdio: "inherit",
      }),
  },
  analyze: {
    ...
    run: () =>
      spawn("node", [join(__dirname, "analyze.mjs"), ...process.argv.slice(3)], {
        stdio: "inherit",
      }),
  },
  diff: {
    ...
    run: () =>
      spawn("node", [join(__dirname, "diff.mjs"), ...process.argv.slice(3)], { stdio: "inherit" }),
  },
  repl: {
  ...
  pipe: {
    ...
    run: () =>
      spawn("node", [join(__dirname, "pipe.mjs"), ...process.argv.slice(3)], { stdio: "inherit" }),
  },
  db: {
    desc: "SQLite database: query, set, get, list, backup",
    usage: "snail db <command> [args]",
    run: () =>
      spawn("node", [join(__dirname, "db.mjs"), ...process.argv.slice(3)], { stdio: "inherit" }),
  },
  scratch: {
    desc: "Scratch pad: quick persistent notes",
    usage: "snail scratch <command> [args]",
    run: () =>
      spawn("node", [join(__dirname, "scratch.mjs"), ...process.argv.slice(3)], {
        stdio: "inherit",
      }),
  },
  si: {
    ...
    run: () =>
      spawn("node", [join(__dirname, "si.mjs"), ...process.argv.slice(3)], { stdio: "inherit" }),
  },
  think: {
    desc: "Structured reasoning: problem → hypothesis → conclusion",
    usage: "snail think <kind> <text> | log | chain | summary",
    run: () =>
      spawn("node", [join(__dirname, "think.mjs"), ...process.argv.slice(3)], { stdio: "inherit" }),
  },
  agenda: {
    ...
    run: () =>
      spawn("node", [join(__dirname, "agenda.mjs"), ...process.argv.slice(3)], {
        stdio: "inherit",
      }),
  },
};
