...
function shouldIgnore(file) {
  return IGNORE_PATTERNS.some(p => 
    file.includes(p) || file.endsWith(p)
  );
}
...
function findUsages(files, symbol, caseSensitive = false) {
  const usages = [];
  const pattern = caseSensitive ? symbol : new RegExp(symbol.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi');
  ...
  for (const file of files) {
    ...
    try {
      ...
      lines.forEach((line, i) => {
        ...
        const matches = caseSensitive ? line.includes(symbol) : line.toLowerCase().includes(symbol.toLowerCase());
        if (matches) {
...
if (cmd === 'tree') {
  ...
  function printTree(dir, prefix = '', depth = 0) {
    ...
    let items;
    try {
      items = readdirSync(dir).filter(f => !f.startsWith('.') && !shouldIgnore(f)).sort();
    } catch (e) { return; }
    ...
    const all = [...dirs.map(d => ({ n: d, t: 'dir' })), ...files.map(f => ({ n: f, t: 'file' }))];
    ...
    all.forEach((item, i) => {
      ...
      if (item.t === 'file') {
        try { size = formatBytes(statSync(full).size); } catch (e) {}
        size = ` (${size})`;
      }
  ...
  console.log(`🔍 Searching for "${symbol}"${caseSensitive ? ' (case-sensitive)' : ''} in ${files.filter(f => !shouldIgnore(f)).length} files...\n`);
  ...
  for (const file of files) {
    ...
    if (result.imports) {
      for (const imp of result.imports) {
        if (!imp.startsWith('.') && !imp.startsWith('/')) { // skip relative imports
  ...
  for (const [ext, data] of Object.entries(byExt).sort((a, b) => b[1].lines - a[1].lines)) {
    console.log(`  ${ext.padEnd(8)} ${data.files.toString().padStart(3)} files  ${data.lines.toString().padStart(6)} lines  ${formatBytes(data.size).padStart(8)}`);
  }
}
