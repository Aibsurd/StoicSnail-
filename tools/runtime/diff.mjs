...
let contentA, contentB, labelA = fileA || 'A', labelB = fileB || 'B';
...
if (statMode) {
  ...
  console.log(`Similarity: ${Math.round(stats.unchanged / Math.max(stats.totalA, stats.totalB) * 100)}%`);
  process.exit(0);
}
...
if (unifiedMode) {
  ...
  diff.removed.forEach(r => {
    const ctx = diff.unchanged.filter(u => u.lineA < r.lineA + showContext && u.lineA > r.lineA - showContext);
...
if (sideBySideMode) {
  ...
  for (let i = 0; i < maxLines; i++) {
    ...
    // Color
    let leftColor = '\x1b[37m', rightColor = '\x1b[37m';
