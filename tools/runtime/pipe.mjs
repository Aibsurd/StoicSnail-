...
function applyTransforms(data, transforms) {
  ...
  for (const t of transforms) {
    ...
    // Array/object methods: .sort .reverse .unique .keys .values .entries
      ...
      result = Array.isArray(result) ? [...new Set(result.map(v => JSON.stringify(v)))].map(v => JSON.parse(v)) : result;
    }
    // Nested: .a.b.c
    ...
    // String transforms
      ...
      if (match) {
        result = typeof result === 'string' ? result.replace(new RegExp(match[1], 'g'), match[2]) : result;
      }
    }
    // Array transforms
    else if (t === '.unique') {
    ...
    // Object transforms
      ...
      result = result && typeof result === 'object' && !Array.isArray(result) ? Object.keys(result) : result;
    }
    else if (t === '.values') {
      result = result && typeof result === 'object' && !Array.isArray(result) ? Object.values(result) : result;
    }
    else if (t === '.entries') {
      result = result && typeof result === 'object' && !Array.isArray(result) ? Object.entries(result) : result;
    }
    // Array properties
    else if (t === '.length') {
      result = Array.isArray(result) ? result.length : (result && typeof result === 'object') ? Object.keys(result).length : String(result).length;
    }
    // Number aggregation
    else if (t === '.sum') {
      result = Array.isArray(result) ? result.filter(v => typeof v === 'number').reduce((a, b) => a + b, 0) : result;
    }
    else if (t === '.avg') {
      ...
      result = typeof result === 'number' ? (n > 0 ? parseFloat(result.toFixed(n)) : Math.round(result)) : result;
    }
    else if (t === '--type') {
