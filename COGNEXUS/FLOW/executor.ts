...
export class FlowExecutor {
  ...
  async createFlow(description: string, taskDefinitions: Omit<Task, "id">[]): Promise<FlowGraph> {
  ...
  async executeFlowInternal(flow: FlowGraph): Promise<void> {
    ...
    while (true) {
      ...
      if (runningTasks.size === 0 && readyTasks.length === 0) {
        ...
        const pendingTasks = Array.from(flow.results.values()).filter(
          (r) => r.status === "pending",
        );
  ...
  private findReadyTasks(flow: FlowGraph, completed: Set<string>, running: Set<string>): Task[] {
    const ready: Task[] = [];
