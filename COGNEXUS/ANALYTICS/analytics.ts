...
export class AnalyticsCollector {
  ...
  private initMetrics(): void {
    ...
    const metricDefs = [
      ...
      {
        name: "deliberation_consensus",
        unit: "score",
        agg: "avg",
        desc: "Average consensus score",
      },
  ...
  recordEvolutionMetrics(data: { changes?: number; pending?: number; score?: number }): void {
  ...
  generateSnapshot(sessionId: string): AnalyticsSnapshot {
    ...
    const snapshot: AnalyticsSnapshot = {
      ...
      cognition: {
        ...
        contextUsagePercent: contextTokens?.value ? (contextTokens.value / 204800) * 100 : 0,
  ...
  private getHistoricalScore(windowMs: number, offsetMs: number): number | null {
    ...
    const points = series.points.filter(
      (p) => p.timestamp > cutoff && p.timestamp < Date.now() - offsetMs,
    );
    if (points.length === 0) return null;
  ...
  getHistoricalData(metricName: string, startTime: number, endTime: number): MetricPoint[] {
