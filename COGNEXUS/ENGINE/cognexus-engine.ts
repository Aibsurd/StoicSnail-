...
export default function registerCogneXusEngine(api: OpenClawPluginApi): void {
  ...
  function computeRetrievalImportance(
    ...
    // Combined score
    return (
      (0.4 * semanticSim + 0.3 * attentionWeight + 0.3 * chunk.importance) * (1 + recencyBonus)
    );
  }
  ...
  api.registerContextEngine(engineId, () => ({
    ...
    async ingest({
      ...
      message,
      isHeartbeat,
    }): Promise<{ ingested: boolean; importance?: number }> {
      if (isHeartbeat) {
        ...
        if (preservedChunks.length > 0) {
          const episodic: EpisodicEntry = {
            ...
            summary: preservedChunks
              ...
              .join(" | "),
            importance:
              preservedChunks.reduce((s, ch) => s + ch.importance, 0) / preservedChunks.length,
    ...
    async detectPatterns(sessionId: string): Promise<unknown[]> {
      ...
      try {
        ...
        return detectedPatterns.filter(
          (p) => (p as { confidence: number }).confidence >= config.patternMinConfidence,
        );
      } catch (error) {
  ...
  api.runtime?.logger?.info?.(
    `CogneXus context engine registered (${engineInfo.name} v${engineInfo.version})`,
  );
}
